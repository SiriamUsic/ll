...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        if not a.can_invite_users:
            await m.reply_text("الصلاحيه مفقودهِ:" + "\n\n» ❌__إضافة مستخدمين__")
            return
        try:
            ...
            if replied.audio or replied.voice:
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await suhu.delete()
                        await m.reply_text(f"🚫 حدث خطأ تأكد من وجود مكالمه مفتوحه اولاَ:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "»قم بالرد على ملف صوتي او اكتبكلمه لابحث عنها**"
                    )
                else:
                    suhu = await c.send_message(chat_id, "🔍**جاري البحث، انتظر قليلاّ...**")
                    ...
                    if search == 0:
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    await suhu.edit("🔄 ** جاري الانضمام الى المكالمه للتشغيل...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 حدث خطأ تأكد من وجود مكالمه مفتوحه اولاَ: `{ep}`")

        else:
            if len(m.command) < 2:
                await m.reply(
                    "» رد على** ملف صوتي ** أو ** اكتب شيئا لابحث عنه.**"
                )
            else:
                suhu = await c.send_message(chat_id, "🔍**جاري البحث، انتظر قليلا...**")
                query = m.text.split(None, 1)[1]
                search = ytsearch(query)
                if search == 0:
                    await suhu.edit("❌ **لم يتم العثور على نتائج.**")
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await suhu.delete()
                                await m.reply_text(f"حدث خطأ تأكد من وجود مكالمه مفتوحه اولا`{ep}`")
